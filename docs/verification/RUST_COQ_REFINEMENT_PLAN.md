...
## Formal Correspondence Layer

Rust↔Coq mechanical correspondence is established in:

    proofs/coq/rust_refinement/KyriotesCsk2RustCoqFormalCorrespondence.v

This file declares Rust functions as Coq `Parameter`s, states `Prop`-valued contracts, proves mechanically-checked implications (rewrite + apply against model theorems), records `Axiom`s with explicit Kani/test backing, and derives unconditional correspondence theorems. The correspondence gate `rust_coq_formal_correspondence_is_closed` is proved by `reflexivity` over the 12-field status record.
