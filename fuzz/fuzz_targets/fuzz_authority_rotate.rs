...
fn fuzz_authority_rotate_surface(data: &[u8]) {
    ...
    let current_epoch = kyriotes_csk2_fuzz::bytes_to_u64(data.get(96..).unwrap_or_default());
    let next_epoch = kyriotes_csk2_fuzz::bytes_to_u64(data.get(104..).unwrap_or_default());
    let chain_hint = kyriotes_csk2_fuzz::bytes_to_u64(data.get(112..).unwrap_or_default());

    let _root_keypair = kyriotes_csk2::kyriotes_csk2::authority::AuthorityRootKeyPair::from_seed(root_seed);
    let _epoch_keypair = kyriotes_csk2::kyriotes_csk2::authority::EpochSigningKeyPair::from_seed(epoch_seed);
    let _alternate_epoch_keypair =
        kyriotes_csk2::kyriotes_csk2::authority::EpochSigningKeyPair::from_seed(alternate_epoch_seed);
...
fuzz_target!(|data: &[u8]| {
    kyriotes_csk2_fuzz::drive_parser_like_targets(data, fuzz_authority_rotate_surface);
});
