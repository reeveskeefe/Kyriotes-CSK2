...
fn fuzz_full_lifecycle_from_bytes(data: &[u8]) {
    // verify() requires cap, proof, state, and transparency_proof which
    // cannot be constructed from raw fuzz bytes — exercise the decode
    // surface across all mutation variants instead.
...
fuzz_target!(|data: &[u8]| {
    kyriotes_csk2_fuzz::drive_parser_like_targets(data, fuzz_full_lifecycle_from_bytes);
});
