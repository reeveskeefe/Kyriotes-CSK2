#![no_main]

use kyriotes_csk2::{
    capability_stamp, hash_policy, issue_capability_and_commit, open, open_and_reseal,
    seal_with_verifier, verify_with_verifier, AuthorityCapabilityTree, AuthorityRootKeyPair,
    AuthorityState, Capability, CapabilityIssuanceProof, CapabilityProof, EpochSigningKeyPair,
    InMemoryTransparencyLog, OpenRequest, RecipientKeyPair, Rights, StubAuthorityVerifier,
    TemporalPolicy,
};
use libfuzzer_sys::fuzz_target;
use rand::{rngs::StdRng, SeedableRng};

const VALIDITY_WINDOW: u64 = 256;

fn seed32(data: &[u8], domain: u8) -> [u8; 32] {
    kyriotes_csk2_fuzz::deterministic_seed32(data, domain)
}

fn base_capability(data: &[u8], policy_hash: [u8; 32]) -> Capability {
    let mut nonce = [0u8; 16];
    nonce.copy_from_slice(&seed32(data.get(8..).unwrap_or_default(), 0xA1)[..16]);
    Capability {
        version: 1,
        subject: "reseal-semantic-subject".to_string(),
        object_id: format!(
            "reseal-semantic-object-{:016x}",
            kyriotes_csk2_fuzz::bytes_to_u64(data)
        ),
        rights: Rights::READ.union(Rights::DECRYPT),
        policy_hash,
        epoch_start: 1,
        epoch_end: 4096,
        delegation_depth: 0,
        parent_stamp: [0u8; 32],
        nonce,
    }
}

fn authority_state(
    tree: &AuthorityCapabilityTree,
    root_pk: [u8; 32],
    authority_id: String,
    epoch: u64,
    prev_epoch_hash: [u8; 32],
) -> AuthorityState {
    AuthorityState {
        authority_root: tree.authority_root(),
        revocation_root: tree.revocation_root(),
        transparency_root: [0u8; 32],
        epoch,
        authority_id,
        root_pk,
        revocation_count: tree.revocation_count(),
        prev_epoch_hash,
    }
}

fn build_capability_proof(
    tree: &AuthorityCapabilityTree,
    cap: &Capability,
    issuance: CapabilityIssuanceProof,
    state: &AuthorityState,
) -> Option<CapabilityProof> {
    Some(CapabilityProof {
        inclusion: tree.inclusion_proof(cap)?,
        non_revocation: tree.non_revocation_witness(&capability_stamp(cap, state)).ok()?,
        issuance,
    })
}

fn issue_commit_and_proof(
    log: &mut InMemoryTransparencyLog,
    tree: &mut AuthorityCapabilityTree,
    cap: &Capability,
    epoch_kp: &EpochSigningKeyPair,
    epoch_cert: &kyriotes_csk2::EpochKeyCert,
    state: &AuthorityState,
) -> Option<(CapabilityProof, kyriotes_csk2::TransparencyStateCommit)> {
    let (issuance, commit) = issue_capability_and_commit(log, tree, cap, epoch_kp, epoch_cert, state).ok()?;
    let proof = build_capability_proof(tree, cap, issuance, &commit.state)?;
    Some((proof, commit))
}

fn temporal_policy_for_open(data: &[u8], open_epoch: u64) -> TemporalPolicy {
    if kyriotes_csk2_fuzz::bytes_to_bool(data.get(24..).unwrap_or_default()) {
        TemporalPolicy::ResealRequired {
            after: open_epoch.saturating_add(1),
        }
    } else {
        TemporalPolicy::Current
    }
}

fn fuzz_open_and_reseal_semantic(data: &[u8]) {
    let data = kyriotes_csk2_fuzz::bounded(data);
    let mut rng = StdRng::from_seed(seed32(data, 0x15));

    let root_kp = AuthorityRootKeyPair::from_seed(seed32(data.get(1..).unwrap_or_default(), 0x25));
    let epoch_kp = EpochSigningKeyPair::from_seed(seed32(data.get(3..).unwrap_or_default(), 0x35));
    let policy_hash = hash_policy(&format!(
        "reseal-semantic-policy-{:016x}",
        kyriotes_csk2_fuzz::bytes_to_u64(data.get(5..).unwrap_or_default())
    ));

    let mut tree = AuthorityCapabilityTree::new();
    let cap = base_capability(data, policy_hash);
    tree.add_capability(&cap);

    let open_epoch = (kyriotes_csk2_fuzz::bytes_to_u64(data.get(13..).unwrap_or_default()) % 32)
        .saturating_add(1);
    let reseal_gap = (data.get(21).copied().unwrap_or(0) as u64 % 6).saturating_add(1);
    let seal_epoch = open_epoch.saturating_add(reseal_gap);
    let authority_id = format!("reseal-semantic-auth-{}", data.first().copied().unwrap_or(0));

    let open_state = authority_state(
        &tree,
        root_kp.verifying_key_bytes(),
        authority_id.clone(),
        open_epoch,
        seed32(data.get(28..).unwrap_or_default(), 0x45),
    );
    let seal_state = authority_state(
        &tree,
        root_kp.verifying_key_bytes(),
        authority_id,
        seal_epoch,
        seed32(data.get(60..).unwrap_or_default(), 0x46),
    );

    let mut log = InMemoryTransparencyLog::new();
    let open_epoch_cert = root_kp.issue_epoch_cert(
        &epoch_kp.verifying_key_bytes(),
        open_epoch,
        VALIDITY_WINDOW,
    );
    let Some((open_proof, open_commit)) = issue_commit_and_proof(
        &mut log,
        &mut tree,
        &cap,
        &epoch_kp,
        &open_epoch_cert,
        &open_state,
    ) else {
        return;
    };

    let original_recipient = RecipientKeyPair::generate(&mut rng);
    let verifier = StubAuthorityVerifier;
    let open_request = OpenRequest {
        object_id: cap.object_id.clone(),
        required_rights: Rights::READ,
        policy_hash,
        epoch: open_commit.state.epoch,
    };

    let original_policy = temporal_policy_for_open(data, open_epoch);

    let original = match seal_with_verifier(
        &verifier,
        &original_recipient.public,
        data,
        &cap,
        &open_proof,
        &open_commit.proof,
        &open_commit.state,
        &open_request,
            original_policy.clone(),
    ) {
        Ok(value) => value,
        Err(_) => return,
    };

    let seal_epoch_cert = root_kp.issue_epoch_cert(
        &epoch_kp.verifying_key_bytes(),
        seal_epoch,
        VALIDITY_WINDOW,
    );
    let Some((seal_proof, seal_commit)) = issue_commit_and_proof(
        &mut log,
        &mut tree,
        &cap,
        &epoch_kp,
        &seal_epoch_cert,
        &seal_state,
    ) else {
        return;
    };

    let seal_request = OpenRequest {
        object_id: cap.object_id.clone(),
        required_rights: Rights::READ,
        policy_hash,
        epoch: seal_commit.state.epoch,
    };
    let new_recipient = RecipientKeyPair::generate(&mut rng);

    let resealed = match open_and_reseal(
        &original_recipient.secret,
        &new_recipient.public,
        &original,
        &cap,
        &open_proof,
        &open_commit.state,
        &seal_proof,
        &seal_commit.proof,
        &seal_commit.state,
        &seal_request,
        TemporalPolicy::Current,
    ) {
        Ok(value) => value,
        Err(_) => return,
    };

    let reseal_proof = match build_capability_proof(&tree, &cap, seal_proof.issuance.clone(), &seal_commit.state) {
        Some(value) => value,
        None => return,
    };

    assert_ne!(original.payload_ciphertext, resealed.payload_ciphertext);
    assert_eq!(resealed.wrappers[0].epoch, seal_commit.state.epoch);
    assert!(
        verify_with_verifier(
            &verifier,
            &resealed,
            &cap,
            &reseal_proof,
            &seal_commit.state,
            &seal_commit.proof,
        )
        .is_ok(),
        "resealed object must verify against the committed seal state"
    );

    assert_eq!(
        open(
            &new_recipient.secret,
            &resealed,
            &cap,
            &reseal_proof,
            &seal_commit.state,
        )
        .expect("new recipient must open resealed object"),
        data
    );
    assert!(
        open(
            &original_recipient.secret,
            &resealed,
            &cap,
            &reseal_proof,
            &seal_commit.state,
        )
        .is_err(),
        "old recipient secret must not decrypt the resealed object"
    );

    let mut tampered_open_state = open_commit.state.clone();
    tampered_open_state.transparency_root[0] ^= 1;
    assert!(
        open_and_reseal(
            &original_recipient.secret,
            &new_recipient.public,
            &original,
            &cap,
            &open_proof,
            &tampered_open_state,
            &seal_proof,
            &seal_commit.proof,
            &seal_commit.state,
            &seal_request,
            TemporalPolicy::Current,
        )
        .is_err(),
        "tampered open state must be rejected"
    );

    let mut tampered_seal_state = seal_commit.state.clone();
    tampered_seal_state.transparency_root[0] ^= 1;
    assert!(
        open_and_reseal(
            &original_recipient.secret,
            &new_recipient.public,
            &original,
            &cap,
            &open_proof,
            &open_commit.state,
            &seal_proof,
            &seal_commit.proof,
            &tampered_seal_state,
            &seal_request,
            TemporalPolicy::Current,
        )
        .is_err(),
        "tampered seal state must be rejected"
    );

    assert!(
        open_and_reseal(
            &original_recipient.secret,
            &new_recipient.public,
            &original,
            &cap,
            &open_proof,
            &open_commit.state,
            &seal_proof,
            &seal_commit.proof,
            &seal_commit.state,
            &seal_request,
            original_policy,
        )
        .is_ok(),
        "original temporal policy must not block a valid reseal path"
    );
}

fuzz_target!(|data: &[u8]| {
    fuzz_open_and_reseal_semantic(data);
});