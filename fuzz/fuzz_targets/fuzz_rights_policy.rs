...
use arc_core::core::rights::Rights;
use libfuzzer_sys::fuzz_target;

fn fuzz_rights_from_bytes(data: &[u8]) {
    // Rights is a u16 newtype. Extract two u16 values from fuzz input.
    let raw_left     = arc_fuzz::bytes_to_u64(data) as u16;
    let raw_right    = arc_fuzz::bytes_to_u64(data.get(8..).unwrap_or_default()) as u16;
    let raw_required = arc_fuzz::bytes_to_u64(data.get(16..).unwrap_or_default()) as u16;
...
fuzz_target!(|data: &[u8]| {
    arc_fuzz::drive_parser_like_targets(data, fuzz_rights_from_bytes);
});
