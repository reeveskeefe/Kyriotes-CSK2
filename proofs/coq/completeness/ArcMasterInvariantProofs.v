...
Record ArcAbstractInvariantBundle := {
...
Definition arc_abstract_invariants_hold
  (bundle : ArcAbstractInvariantBundle)
  : bool :=
...
Definition bundle_from_verified_open
  ...
  {|
    ...
    bundle_refinement_map_gate := arc_refinement_map_has_core_coverage entries
  |}.

Definition arc_master_open_invariant
  (obj : ArcObject)
  (cap : Capability)
  (state : AuthorityState)
  (transition : AuthorityStateTransition)
  ...
  arc_refinement_map_has_core_coverage entries.

Theorem abstract_invariants_hold_implies_key_gate :
  forall bundle,
    arc_abstract_invariants_hold bundle = true ->
    bundle_key_gate bundle = true.
Proof.
  intros bundle H.
  unfold arc_abstract_invariants_hold in H.
...
Theorem verified_open_builds_core_safety_bundle :
  forall obj cap state transition wrapper step entries,
    ...
    arc_refinement_map_has_core_coverage entries = true ->
    arc_abstract_invariants_hold
...
Proof.
  ...
  unfold arc_abstract_invariants_hold.
...
Theorem master_open_invariant_implies_verified_open :
  forall obj cap state transition wrapper step entries,
    arc_master_open_invariant obj cap state transition wrapper step entries = true ->
    verify_open_context obj cap state = true.
Proof.
  intros obj cap state transition wrapper step entries H.
  unfold arc_master_open_invariant in H.
  repeat rewrite andb_true_iff in H.
...
Proof.
  ...
  - apply master_open_invariant_implies_transition_open with
    ...
    exact H_master.
  - unfold arc_master_open_invariant in H_master.
...
Theorem arc_abstract_protocol_reaches_high_invariant_coverage :
  forall obj cap state transition wrapper step,
    arc_master_open_invariant
      obj cap state transition wrapper step arc_reference_refinement_map = true ->
    arc_abstract_invariants_hold
