...
Record ArcLifecycleStep := {
  lifecycle_event : ArcLifecycleEvent;
  lifecycle_from : ArcLifecycleState;
  lifecycle_to : ArcLifecycleState
}.

Definition lifecycle_epoch_no_regression (step : ArcLifecycleStep) : bool :=
  Nat.leb (lifecycle_epoch (lifecycle_from step)) (lifecycle_epoch (lifecycle_to step)).

Definition lifecycle_issue_valid (step : ArcLifecycleStep) : bool :=
  negb (lifecycle_cap_issued (lifecycle_from step)) &&
  lifecycle_cap_issued (lifecycle_to step) &&
  lifecycle_epoch_no_regression step.

Definition lifecycle_seal_valid (step : ArcLifecycleStep) : bool :=
  lifecycle_cap_issued (lifecycle_from step) &&
  lifecycle_object_sealed (lifecycle_to step) &&
  lifecycle_epoch_no_regression step.

Definition lifecycle_verify_valid (step : ArcLifecycleStep) : bool :=
  lifecycle_object_sealed (lifecycle_from step) &&
  lifecycle_verified (lifecycle_to step) &&
  lifecycle_epoch_no_regression step.

Definition lifecycle_open_valid (step : ArcLifecycleStep) : bool :=
  lifecycle_verified (lifecycle_from step) &&
  negb (lifecycle_revoked (lifecycle_from step)) &&
  lifecycle_opened (lifecycle_to step) &&
  lifecycle_epoch_no_regression step.

Definition lifecycle_revoke_valid (step : ArcLifecycleStep) : bool :=
  lifecycle_cap_issued (lifecycle_from step) &&
  lifecycle_revoked (lifecycle_to step) &&
  lifecycle_epoch_no_regression step.

Definition lifecycle_reject_valid (step : ArcLifecycleStep) : bool :=
  lifecycle_revoked (lifecycle_from step) &&
  negb (lifecycle_opened (lifecycle_to step)) &&
  lifecycle_epoch_no_regression step.

Definition lifecycle_rotate_valid (step : ArcLifecycleStep) : bool :=
  Nat.ltb (lifecycle_epoch (lifecycle_from step)) (lifecycle_epoch (lifecycle_to step)).

Definition lifecycle_rewrap_valid (step : ArcLifecycleStep) : bool :=
  lifecycle_object_sealed (lifecycle_from step) &&
  lifecycle_rewrapped (lifecycle_to step) &&
  lifecycle_epoch_no_regression step.

Definition lifecycle_step_valid (step : ArcLifecycleStep) : bool :=
...
Fixpoint lifecycle_trace_valid (trace : list ArcLifecycleStep) : bool :=
  match trace with
  | [] => true
