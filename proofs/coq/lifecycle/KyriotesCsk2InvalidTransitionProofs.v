...
Definition invalid_epoch_regression (step : KyriotesCsk2MachineStep) : bool :=
  Nat.ltb (machine_epoch (step_to step)) (machine_epoch (step_from step)).

Definition invalid_same_epoch_rotation (step : KyriotesCsk2MachineStep) : bool :=
  match step_operation step with
  | OpRotate => Nat.eqb (machine_epoch (step_from step)) (machine_epoch (step_to step))
  | _ => false
  end.

Definition invalid_revocation_root_regression (step : KyriotesCsk2MachineStep) : bool :=
  match step_operation step with
  | OpRevoke => Nat.ltb (machine_revocation_root (step_to step)) (machine_revocation_root (step_from step))
  | _ => false
  end.

Definition invalid_sealed_count_regression (step : KyriotesCsk2MachineStep) : bool :=
  match step_operation step with
  | OpSeal => Nat.ltb (machine_sealed_count (step_to step)) (machine_sealed_count (step_from step))
  | _ => false
  end.

Definition invalid_open_count_regression (step : KyriotesCsk2MachineStep) : bool :=
  match step_operation step with
  | OpOpen => Nat.ltb (machine_open_count (step_to step)) (machine_open_count (step_from step))
  | _ => false
  end.

Definition invalid_machine_step (step : KyriotesCsk2MachineStep) : bool :=
