...
Record ArcPrimitiveBreakVector := {
...
Definition primitive_break_any
  (breaks : ArcPrimitiveBreakVector)
  : bool :=
...
Record ArcReductionAttempt := {
  ...
  reduction_primitive_breaks : ArcPrimitiveBreakVector
}.

Definition reduction_authorized
  (attempt : ArcReductionAttempt)
  : bool :=
...
Definition reduction_arc_break_possible
  (attempt : ArcReductionAttempt)
  : bool :=
  reduction_authorized attempt ||
  primitive_break_any (reduction_primitive_breaks attempt).

Definition reduction_unauthorized_arc_break
  (attempt : ArcReductionAttempt)
  : bool :=
  negb (reduction_authorized attempt) &&
  reduction_arc_break_possible attempt.
...
Theorem unauthorized_arc_break_implies_primitive_break :
  forall attempt,
    reduction_unauthorized_arc_break attempt = true ->
    primitive_break_any (reduction_primitive_breaks attempt) = true.
Proof.
  intros attempt H.
  unfold reduction_unauthorized_arc_break in H.
  apply andb_true_iff in H.
  destruct H as [H_not_authorized H_break_possible].
  unfold reduction_arc_break_possible in H_break_possible.
...
Theorem no_primitive_break_and_not_authorized_blocks_arc_break :
  forall attempt,
    primitive_break_any (reduction_primitive_breaks attempt) = false ->
    reduction_authorized attempt = false ->
    reduction_arc_break_possible attempt = false.
Proof.
  intros attempt H_no_break H_not_auth.
  unfold reduction_arc_break_possible.
  rewrite H_not_auth.
  simpl.
  exact H_no_break.
Qed.

Theorem arc_break_possible_implies_authorized_or_primitive_break :
  forall attempt,
    reduction_arc_break_possible attempt = true ->
    reduction_authorized attempt = true \/
    primitive_break_any (reduction_primitive_breaks attempt) = true.
Proof.
  intros attempt H.
  unfold reduction_arc_break_possible in H.
  destruct (reduction_authorized attempt) eqn:H_auth.
  - left.
    reflexivity.
  - simpl in H.
...
Theorem arc_break_possible_implies_authorized_or_positive_symbolic_advantage :
  forall attempt,
    reduction_arc_break_possible attempt = true ->
    reduction_authorized attempt = true \/
...
Proof.
  ...
  pose proof (arc_break_possible_implies_authorized_or_primitive_break attempt H) as H_cases.
...
Theorem zero_symbolic_advantage_and_arc_break_implies_authorized :
  forall attempt,
    reduction_symbolic_advantage attempt = 0 ->
    reduction_arc_break_possible attempt = true ->
    reduction_authorized attempt = true.
Proof.
  ...
  unfold reduction_arc_break_possible in H_break.
  destruct (reduction_authorized attempt) eqn:H_auth.
  - reflexivity.
  - simpl in H_break.
...
Record ArcCryptoReductionCoverage := {
  ...
  crypto_arc_break_possible_model : bool;
  ...
  crypto_no_break_and_not_authorized_blocks_arc_break : bool;
  crypto_arc_break_implies_authorized_or_break : bool;
  crypto_arc_break_implies_authorized_or_positive_advantage : bool;
...
Definition arc_crypto_reduction_coverage_complete
  (coverage : ArcCryptoReductionCoverage)
  : bool :=
  ...
  crypto_arc_break_possible_model coverage &&
  ...
  crypto_no_break_and_not_authorized_blocks_arc_break coverage &&
  crypto_arc_break_implies_authorized_or_break coverage &&
  crypto_arc_break_implies_authorized_or_positive_advantage coverage &&
...
Definition arc_current_crypto_reduction_coverage : ArcCryptoReductionCoverage :=
  {|
    ...
    crypto_arc_break_possible_model := true;
    ...
    crypto_no_break_and_not_authorized_blocks_arc_break := true;
    crypto_arc_break_implies_authorized_or_break := true;
    crypto_arc_break_implies_authorized_or_positive_advantage := true;
...
Definition arc_crypto_reduction_coverage_score
  (coverage : ArcCryptoReductionCoverage)
  : nat :=
  ...
  (if crypto_arc_break_possible_model coverage then 1 else 0) +
  ...
  (if crypto_no_break_and_not_authorized_blocks_arc_break coverage then 1 else 0) +
  (if crypto_arc_break_implies_authorized_or_break coverage then 1 else 0) +
  (if crypto_arc_break_implies_authorized_or_positive_advantage coverage then 1 else 0) +
...
Definition arc_crypto_reduction_coverage_total : nat := 41.

Definition arc_crypto_reduction_coverage_is_100_percent
  (coverage : ArcCryptoReductionCoverage)
  : bool :=
  Nat.eqb
    (arc_crypto_reduction_coverage_score coverage)
    arc_crypto_reduction_coverage_total.

Theorem current_crypto_reduction_coverage_complete :
  arc_crypto_reduction_coverage_complete arc_current_crypto_reduction_coverage = true.
Proof.
  reflexivity.
Qed.

Theorem current_crypto_reduction_coverage_score_is_total :
  arc_crypto_reduction_coverage_score arc_current_crypto_reduction_coverage =
  arc_crypto_reduction_coverage_total.
Proof.
  reflexivity.
Qed.
...
Theorem arc_crypto_reduction_symbolic_layer_closed :
  arc_crypto_reduction_coverage_complete arc_current_crypto_reduction_coverage = true /\
  arc_crypto_reduction_coverage_is_100_percent arc_current_crypto_reduction_coverage = true /\
  arc_abstract_invariant_coverage_is_100_percent arc_current_abstract_invariant_coverage = true /\
  arc_design_model_coverage_is_100_percent arc_current_design_model_coverage = true /\
...
Theorem arc_crypto_reduction_100_is_symbolic_not_probability_claim :
  arc_crypto_reduction_coverage_is_100_percent arc_current_crypto_reduction_coverage = true /\
  obligations_all_satisfied current_arc_refinement_obligations = false.
Proof.
  split.
