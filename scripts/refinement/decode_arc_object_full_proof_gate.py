...
ARTIFACT_PATH = PROJECT_ROOT / "tests" / "refinement" / "decode_arc_object_full_proof_status.json"

HARNESSES = [
    "decode_arc_object_rejects_empty_input",
    "decode_arc_object_rejects_one_byte_input",
    "decode_arc_object_rejects_two_byte_input",
    "decode_arc_object_rejects_tiny_malformed_input",
    "decode_arc_object_rejects_bounded_malformed_input",
    "decode_arc_object_malformed_rejection_is_deterministic",
]
...
def update_inventory(proven: bool) -> None:
    ...
    for entry in payload.get("entries", []):
        if entry.get("id") == "codec.decode_arc_object":
            entry["mechanically_checked"] = True
            entry["mechanically_proven"] = proven
            if proven:
                entry["boundary"] = (
                    "decode_arc_object has verifier-backed Kani proof evidence for "
                ...
                entry["boundary"] = (
                    "decode_arc_object has mechanical check evidence, but verifier-backed "
    ...
    if proven:
        payload["boundary"] = (
            ...
            "context_hash plus decode_arc_object verifier-backed proof lanes complete "
...
def main() -> int:
    ...
    status = {
        "schema": "arc.decode_arc_object.full_mechanical_proof_status.v1",
        "target": "src/encoding/wire.rs::decode_arc_object",
        "proof_tool": "kani",
        ...
        "harness_results": [],
        "boundary": (
            "Full decode_arc_object proof requires all Kani malformed-input parser "
    ...
    if all_passed:
        status["boundary"] = (
            "All decode_arc_object Kani parser-safety harnesses passed. "
            "decode_arc_object is marked mechanically proven for bounded malformed "
        ...
        print("PASS: all decode_arc_object Kani parser-safety harnesses succeeded.")
    else:
        status["reason"] = "At least one required Kani harness failed; target remains checked but not proven."
        update_inventory(proven=False)
        print("OPEN: at least one decode_arc_object Kani harness failed; target remains checked but not proven.")
