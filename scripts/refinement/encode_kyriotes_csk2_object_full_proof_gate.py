...
ARTIFACT_PATH = PROJECT_ROOT / "tests" / "refinement" / "encode_kyriotes_csk2_object_full_proof_status.json"

HARNESSES = [
    "encode_kyriotes_csk2_object_is_deterministic_for_equal_input",
    "encode_kyriotes_csk2_object_returns_non_empty_bytes",
    "encode_kyriotes_csk2_object_starts_with_kyriotes_csk2_magic",
    "encode_kyriotes_csk2_object_version_one_layout_is_stable",
    "encode_kyriotes_csk2_object_binds_object_id",
]
...
def update_inventory(proven: bool) -> None:
    ...
    for entry in payload.get("entries", []):
        if entry.get("id") == "codec.encode_kyriotes_csk2_object":
            entry["mechanically_checked"] = True
            entry["mechanically_proven"] = proven
            if proven:
                entry["boundary"] = (
                    "encode_kyriotes_csk2_object has verifier-backed Kani proof evidence for "
                    "deterministic encoding, non-empty output, Kyriotēs-CSK2 magic/header stability, "
                ...
                entry["boundary"] = (
                    "encode_kyriotes_csk2_object has mechanical check evidence, but verifier-backed "
    ...
    if proven:
        payload["boundary"] = (
            ...
            "context_hash, decode_kyriotes_csk2_object, and encode_kyriotes_csk2_object verifier-backed "
...
def main() -> int:
    ...
    status = {
        "schema": "kyriotes_csk2.encode_kyriotes_csk2_object.full_mechanical_proof_status.v2",
        "target": "src/encoding/wire.rs::encode_kyriotes_csk2_object",
        "proof_tool": "kani",
        ...
        "harness_results": [],
        "boundary": (
            "Full encode_kyriotes_csk2_object proof requires all Kani encoding-surface harnesses to pass. "
            "This proves deterministic minimal-object encoding, non-empty output, Kyriotēs-CSK2 magic/header "
    ...
    if all_passed:
        status["boundary"] = (
            "All encode_kyriotes_csk2_object Kani encoding-surface harnesses passed. encode_kyriotes_csk2_object "
            "is marked mechanically proven for deterministic minimal-object encoding, non-empty "
            "output, Kyriotēs-CSK2 magic/header stability, version layout stability, and object_id binding. "
        ...
        print("PASS: all encode_kyriotes_csk2_object Kani encoding-surface harnesses succeeded.")
    else:
        status["reason"] = "At least one required Kani harness failed; target remains checked but not proven."
        update_inventory(proven=False)
        print("OPEN: at least one encode_kyriotes_csk2_object Kani harness failed; target remains checked but not proven.")
