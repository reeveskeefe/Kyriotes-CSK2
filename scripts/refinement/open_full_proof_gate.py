...
ARTIFACT_PATH = PROJECT_ROOT / "tests" / "refinement" / "open_full_proof_status.json"

HARNESSES = [
    "open_with_verifier_propagates_authority_rejection",
    "open_with_verifier_rejection_is_deterministic",
    "open_with_verifier_rejects_before_capability_validation_can_succeed",
    "open_with_verifier_rejects_invalid_authority_surface_without_panic",
]
...
def update_inventory(proven: bool) -> None:
    ...
    for entry in payload.get("entries", []):
        if entry.get("id") == "engine.open":
            entry["mechanically_checked"] = True
            entry["mechanically_proven"] = proven
            if proven:
                entry["boundary"] = (
                    "open has verifier-backed Kani proof evidence for fail-closed "
                    "authority-verifier rejection propagation, deterministic rejection, "
                    "early rejection before capability validation/decryption, and no-panic "
                    "behavior on a bounded invalid authority surface. Full cryptographic "
                    "open/decryption equivalence remains outside this narrow proof claim."
                )
            else:
                entry["boundary"] = (
                    "open has mechanical check evidence, but verifier-backed fail-closed "
...
def main() -> int:
    ...
    status = {
        ...
        "harness_results": [],
        "boundary": (
            "Full open proof requires all Kani fail-closed authority-verifier harnesses "
            "to pass. This proves rejection propagation and deterministic rejection for "
            "bounded invalid surfaces, not full cryptographic open/decryption equivalence."
        ),
    }
    ...
    if all_passed:
        status["boundary"] = (
            "All open Kani fail-closed authority-verifier harnesses passed. open "
            "is marked mechanically proven for narrow rejection propagation and deterministic "
            "failure behavior. Full cryptographic open/decryption equivalence remains outside "
            "this narrow proof claim."
        )
        update_inventory(proven=True)
        print("PASS: all open Kani fail-closed harnesses succeeded.")
    else:
        status["reason"] = "At least one required Kani harness failed; target remains checked but not proven."
        update_inventory(proven=False)
        print("OPEN: at least one open Kani harness failed; target remains checked but not proven.")
