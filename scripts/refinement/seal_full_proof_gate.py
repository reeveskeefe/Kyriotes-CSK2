...
ARTIFACT_PATH = PROJECT_ROOT / "tests" / "refinement" / "seal_full_proof_status.json"

HARNESSES = [
    "seal_with_verifier_propagates_authority_rejection",
    "seal_with_verifier_rejection_is_deterministic",
    "seal_with_verifier_rejects_before_capability_validation_can_succeed",
    "seal_with_verifier_rejects_invalid_authority_surface_without_panic",
]
...
def update_inventory(proven: bool) -> None:
    ...
    for entry in payload.get("entries", []):
        if entry.get("id") == "engine.seal":
            entry["mechanically_checked"] = True
            entry["mechanically_proven"] = proven
            if proven:
                entry["boundary"] = (
                    "seal has verifier-backed Kani proof evidence for fail-closed "
                    "authority-verifier rejection propagation, deterministic rejection, "
                    "early rejection before capability validation/encryption, and no-panic "
                    "behavior on a bounded invalid authority surface. Full cryptographic "
                    "sealing equivalence remains outside this narrow proof claim."
                )
            else:
                entry["boundary"] = (
                    "seal has mechanical check evidence, but verifier-backed fail-closed "
    ...
    if proven:
        payload["boundary"] = (
            ...
            "context_hash, decode_kyriotes_csk2_object, encode_kyriotes_csk2_object, verify, and seal "
...
def main() -> int:
    ...
    status = {
        "schema": "kyriotes_csk2.seal.full_mechanical_proof_status.v1",
        "target": "src/kyriotes_csk2/engine.rs::seal_with_verifier",
        "proof_tool": "kani",
        ...
        "harness_results": [],
        "boundary": (
            "Full seal proof requires all Kani fail-closed authority-verifier harnesses "
            "to pass. This proves rejection propagation and deterministic rejection for "
            "bounded invalid surfaces, not full cryptographic sealing equivalence."
        ),
    }
    ...
    if all_passed:
        status["boundary"] = (
            "All seal Kani fail-closed authority-verifier harnesses passed. seal "
            "is marked mechanically proven for narrow rejection propagation and deterministic "
            "failure behavior. Full cryptographic sealing equivalence remains outside "
            "this narrow proof claim."
        )
        update_inventory(proven=True)
        print("PASS: all seal Kani fail-closed harnesses succeeded.")
    else:
        status["reason"] = "At least one required Kani harness failed; target remains checked but not proven."
        update_inventory(proven=False)
        print("OPEN: at least one seal Kani harness failed; target remains checked but not proven.")
