...
ARTIFACT_PATH = PROJECT_ROOT / "tests" / "refinement" / "verify_full_proof_status.json"

HARNESSES = [
    "verify_with_verifier_propagates_authority_rejection",
    "verify_with_verifier_rejection_is_deterministic",
    "verify_with_verifier_rejects_before_capability_proof_can_succeed",
    "verify_with_verifier_rejects_invalid_authority_surface_without_panic",
]
...
def update_inventory(proven: bool) -> None:
    ...
    for entry in payload.get("entries", []):
        if entry.get("id") == "engine.verify":
            entry["mechanically_checked"] = True
            entry["mechanically_proven"] = proven
            if proven:
                entry["boundary"] = (
                    "verify has verifier-backed Kani proof evidence for fail-closed "
                    "authority-verifier rejection propagation, deterministic rejection, "
                    "early rejection before capability proof success, and no-panic behavior "
                    "on a bounded invalid authority surface. Full cryptographic verification "
                    "equivalence remains outside this narrow proof claim."
                )
            else:
                entry["boundary"] = (
                    "verify has mechanical check evidence, but verifier-backed fail-closed "
    ...
    if proven:
        payload["boundary"] = (
            ...
            "context_hash, decode_kyriotes_csk2_object, encode_kyriotes_csk2_object, and verify verifier-backed "
...
def main() -> int:
    ...
    status = {
        "schema": "kyriotes_csk2.verify.full_mechanical_proof_status.v1",
        "target": "src/kyriotes_csk2/engine.rs::verify_with_verifier",
        "proof_tool": "kani",
        ...
        "harness_results": [],
        "boundary": (
            "Full verify proof requires all Kani fail-closed authority-verifier harnesses "
            "to pass. This proves rejection propagation and deterministic rejection for "
            "bounded invalid surfaces, not full cryptographic verification equivalence."
        ),
    }
    ...
    if all_passed:
        status["boundary"] = (
            "All verify Kani fail-closed authority-verifier harnesses passed. verify "
            "is marked mechanically proven for narrow rejection propagation and deterministic "
            "failure behavior. Full cryptographic verification equivalence remains outside "
            "this narrow proof claim."
        )
        update_inventory(proven=True)
        print("PASS: all verify Kani fail-closed harnesses succeeded.")
    else:
        status["reason"] = "At least one required Kani harness failed; target remains checked but not proven."
        update_inventory(proven=False)
        print("OPEN: at least one verify Kani harness failed; target remains checked but not proven.")
