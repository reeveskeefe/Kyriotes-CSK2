...
//!
//! Kyriotēs-CSK2 separates the long-lived offline authority root key from the per-epoch
//! online signing key:
//!
//! ```text
//! cert_e = Sign(sk_A_off, H("KYRIOTES-CSK2-EPOCH-KEY-v1" || pk_A_e || e || validity_window))
//! sigma_e = Sign(sk_A_e,  H("KYRIOTES-CSK2-EPOCH-ROOT-v1" || R_e || Rev_e || e || prev_epoch_hash))
//! ```
//!
...
///
/// Format: `"KYRIOTES-CSK2-EPOCH-KEY-v1" || pk_A_e || epoch_le64 || validity_window_le64`
pub fn epoch_cert_signing_message(
    ...
    msg.extend_from_slice(b"KYRIOTES-CSK2-EPOCH-KEY-v1");
...
///
/// Format: `"KYRIOTES-CSK2-CAPABILITY-ISSUE-v1" || leaf_hash || authority_root || epoch_le64`
///
...
pub fn capability_issuance_signing_message(
    ...
    msg.extend_from_slice(b"KYRIOTES-CSK2-CAPABILITY-ISSUE-v1");
...
///
/// Format: `"KYRIOTES-CSK2-EPOCH-ROOT-v2" || R_e || Rev_e || T_e || epoch_le64 || prev_epoch_hash`
///
...
pub fn epoch_root_signing_message(
    ...
    msg.extend_from_slice(b"KYRIOTES-CSK2-EPOCH-ROOT-v2");
    msg.extend_from_slice(authority_root);
...
///
/// Format: `"KYRIOTES-CSK2-COMPROMISE-v1" || pk_A_e_bad || e_bad_le64 || R_recover`
pub fn compromise_notice_signing_message(
    ...
    msg.extend_from_slice(b"KYRIOTES-CSK2-COMPROMISE-v1");
...
pub fn verify_epoch_cert(
    ...
    let root_pk = VerifyingKey::from_bytes(root_pk_bytes)
        .map_err(|_| KyriotesCsk2Error::AuthorityState("invalid authority root public key"))?;
    ...
    root_pk
        ...
        .map_err(|_| KyriotesCsk2Error::AuthorityState("epoch key certificate signature invalid"))
}
...
pub fn verify_epoch_root_sig(
    ...
    let epoch_pk = VerifyingKey::from_bytes(epoch_pk_bytes)
        .map_err(|_| KyriotesCsk2Error::AuthorityState("invalid epoch public key"))?;
    ...
    epoch_pk
        .verify_strict(&msg, &sig)
        .map_err(|_| KyriotesCsk2Error::AuthorityState("epoch root signature invalid"))
}
...
pub fn verify_compromise_notice(
    ...
    root_pk
        .verify_strict(&msg, &sig)
        .map_err(|_| KyriotesCsk2Error::AuthorityState("compromise notice signature invalid"))
}
