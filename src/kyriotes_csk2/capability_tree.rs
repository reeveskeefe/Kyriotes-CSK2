...
//!
//! Kyriotēs-CSK2 authorities maintain two sets per epoch:
//!
...
impl AuthorityCapabilityTree {
    ...
    pub fn non_revocation_witness(
        &self,
        stamp: &[u8; 32],
    ) -> Result<NonRevocationWitness, KyriotesCsk2Error> {
        let pos = self.revoked_stamps.partition_point(|s| s < stamp);
        if pos < self.revoked_stamps.len() && &self.revoked_stamps[pos] == stamp {
...
pub fn verify_capability_issuance(
    ...
    let epoch_pk = VerifyingKey::from_bytes(&proof.epoch_cert.epoch_pk).map_err(|_| {
        KyriotesCsk2Error::InvalidCapability("invalid epoch public key in issuance proof")
    })?;
    ...
    epoch_pk
        ...
        .map_err(|_| KyriotesCsk2Error::InvalidCapability("capability issuance signature invalid"))
}
...
mod tests {
    ...
    fn inclusion_proof_rejects_wrong_root() {
        ...
        assert!(matches!(
            err,
            KyriotesCsk2Error::InvalidCapability("capability Merkle inclusion proof root mismatch")
        ));
    }
    ...
    fn capability_issuance_proof_rejects_tampered_capability() {
        ...
        assert!(matches!(
            err,
            KyriotesCsk2Error::InvalidCapability("capability issuance signature invalid")
        ));
    }
