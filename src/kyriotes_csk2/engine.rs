...
use super::model::{
    ...
    KyriotesCsk2Object, OpenRequest, RecipientPublicKey, RecipientSecretKey, TransparencyProof,
...
pub fn validate_capability(
    ...
    verify_capability_inclusion(cap, &proof.inclusion, &state.authority_root)
        .map_err(|_| KyriotesCsk2Error::InvalidCapability("merkle inclusion failed"))?;

    let expected_stamp = capability_stamp(cap, state);
    if proof.non_revocation.stamp != expected_stamp {
        return Err(KyriotesCsk2Error::InvalidCapability("capability revoked"));
    }
    ...
    .map_err(|_| KyriotesCsk2Error::InvalidCapability("capability revoked"))?;
    ...
    .map_err(|_| KyriotesCsk2Error::InvalidCapability("invalid issuance signature"))?;

    if cap.object_id != req.object_id {
        return Err(KyriotesCsk2Error::InvalidCapability("object mismatch"));
    }
    if !cap.rights.contains_all(req.required_rights) {
        return Err(KyriotesCsk2Error::InvalidCapability("insufficient rights"));
    }
    if cap.policy_hash != req.policy_hash {
        return Err(KyriotesCsk2Error::InvalidCapability("policy hash mismatch"));
    }
...
fn authority_digest(state: &AuthorityState, policy_hash: [u8; 32]) -> [u8; 32] {
    ...
    hasher.update(b"KYRIOTES-CSK2-AUTHORITY-DIGEST-v1");
...
fn derive_kek(
    ...
    info.extend_from_slice(b"KYRIOTES-CSK2-KEK-v1");
...
fn wrap_dek(
    ...
    cipher
        ...
        .map_err(|_| KyriotesCsk2Error::Crypto("failed to wrap DEK"))
}
...
fn unwrap_dek(
    ...
    wrapped_dek: &[u8],
    aad: &[u8],
) -> Result<[u8; 32], KyriotesCsk2Error> {
    let cipher = ChaCha20Poly1305::new(Key::from_slice(&kek));
    let dec = cipher
        .decrypt(
            Nonce::from_slice(&nonce),
        ...
        .map_err(|_| KyriotesCsk2Error::Crypto("failed to unwrap DEK"))?;

    if dec.len() != 32 {
        return Err(KyriotesCsk2Error::Crypto("invalid unwrapped DEK length"));
    }
...
fn payload_encrypt(
    ...
    cipher
        ...
        .map_err(|_| KyriotesCsk2Error::Crypto("payload encryption failed"))
}
...
fn payload_decrypt(
    ...
    cipher
        ...
        .map_err(|_| KyriotesCsk2Error::Crypto("payload decryption failed"))
}

fn authority_aad(
    object: &KyriotesCsk2Object,
    state: &AuthorityState,
    ctx: [u8; 32],
    ...
    aad.extend_from_slice(b"KYRIOTES-CSK2-AUTHORITY-AAD-v2");
...
fn payload_aad(object: &KyriotesCsk2Object) -> Vec<u8> {
    let mut aad = Vec::new();
    aad.extend_from_slice(b"KYRIOTES-CSK2-PAYLOAD-AAD-v1");
    put_str(&mut aad, &object.object_id);
    put_rights(&mut aad, object.required_rights);
    put_bytes(&mut aad, &object.policy_hash);
...
fn select_required_wrapper<'a>(
    object: &'a KyriotesCsk2Object,
    state: &AuthorityState,
) -> Result<(u64, &'a AuthorityWrapper), KyriotesCsk2Error> {
    let e_open = state.epoch;
    if !object.temporal_policy.accepts(e_open, object.seal_epoch) {
        return Err(KyriotesCsk2Error::TemporalRejected);
    }
    ...
    let wrapper = object
        ...
        .ok_or(KyriotesCsk2Error::MissingWrapper)?;

    Ok((e_req, wrapper))
}

fn open_request_for_object(object: &KyriotesCsk2Object, epoch: u64) -> OpenRequest {
    OpenRequest {
...
pub fn seal_with_verifier<V: AuthorityVerifier>(
    ...
    let verified_state = verifier.verify_state(state, transparency_proof)?;
    seal_verified(
        recipient_pk,
        message,
        cap,
...
pub fn seal_verified(
    ...
    proof: &CapabilityProof,
    transparency_proof: &TransparencyProof,
    verified_state: &VerifiedAuthorityState,
    req: &OpenRequest,
    temporal_policy: TemporalPolicy,
) -> Result<KyriotesCsk2Object, KyriotesCsk2Error> {
    ...
    let mut object = KyriotesCsk2Object {
        version: 1,
        suite: "KYRIOTES-CSK2-DEV-CHACHA20POLY1305-HKDF-SHA256".to_string(),
...
/// [`KyriotesCsk2Object`]'s wrapper, so the object is immediately verifiable against the
...
pub fn seal_and_commit<V: AuthorityVerifier, L: TransparencyLog>(
    ...
    req: &OpenRequest,
    temporal_policy: TemporalPolicy,
) -> Result<(KyriotesCsk2Object, TransparencyStateCommit), KyriotesCsk2Error> {
...
fn unwrap_dek_for_epoch(
    ...
    if ctx != wrapper.context_hash {
        return Err(KyriotesCsk2Error::AuthorityState("context hash mismatch"));
    }
...
pub fn add_epoch_wrapper_with_verifier<V: AuthorityVerifier>(
    ...
    let from_verified = verifier.verify_state(from_state, &from_wrapper.transparency_proof)?;
    let to_verified = verifier.verify_state(to_state, to_transparency_proof)?;
    add_epoch_wrapper_verified(
        recipient_sk,
        recipient_pk,
        object,
        cap,
...
#[allow(clippy::too_many_arguments)]
pub fn add_epoch_wrapper_verified(
    recipient_sk: &RecipientSecretKey,
    recipient_pk: &RecipientPublicKey,
    object: &mut KyriotesCsk2Object,
    cap: &Capability,
    proof: &CapabilityProof,
    from_verified: &VerifiedAuthorityState,
    to_verified: &VerifiedAuthorityState,
    to_transparency_proof: &TransparencyProof,
) -> Result<(), KyriotesCsk2Error> {
    let from_state = from_verified.as_state();
    let to_state = to_verified.as_state();
    let from_wrapper = object
        .wrappers
        .iter()
        .find(|w| w.epoch == from_state.epoch)
...
pub fn revoke_capability(
    ...
    revoke_epoch: u64,
) -> Result<AuthorityState, KyriotesCsk2Error> {
...
/// Public verifiability check for an Kyriotēs-CSK2 object — spec §2 Verify.
///
...
pub fn verify_with_verifier<V: AuthorityVerifier>(
    ...
    let verified_state = verifier.verify_state(state, transparency_proof)?;
    verify_verified(object, cap, proof, &verified_state)
}
...
/// Reseal an Kyriotēs-CSK2 object with a fresh DEK and (optionally) a new recipient key
...
pub fn open_and_reseal_with_verifier<V: AuthorityVerifier>(
    ...
    let open_wrapper = object
        .wrappers
        .iter()
        .find(|w| w.epoch == open_state.epoch)
        .ok_or(KyriotesCsk2Error::MissingWrapper)?;
    let open_verified = verifier.verify_state(open_state, &open_wrapper.transparency_proof)?;
    let seal_verified = verifier.verify_state(seal_state, seal_transparency_proof)?;

    open_and_reseal_verified(
        recipient_sk,
        recipient_pk_new,
        object,
        cap,
...
#[allow(clippy::too_many_arguments)]
pub fn open_and_reseal_verified(
    recipient_sk: &RecipientSecretKey,
    recipient_pk_new: &RecipientPublicKey,
    object: &KyriotesCsk2Object,
    cap: &Capability,
    open_proof: &CapabilityProof,
    open_verified_state: &VerifiedAuthorityState,
    seal_proof: &CapabilityProof,
    seal_transparency_proof: &TransparencyProof,
    seal_verified_state: &VerifiedAuthorityState,
    seal_req: &OpenRequest,
    new_temporal_policy: TemporalPolicy,
) -> Result<KyriotesCsk2Object, KyriotesCsk2Error> {
    let open_state = open_verified_state.as_state();
    let (e_req, wrapper) = select_required_wrapper(object, open_state)?;
    let plaintext = open_verified(
        recipient_sk,
        object,
        cap,
        open_proof,
...
/// Reseal an Kyriotēs-CSK2 object and atomically commit the new seal state to the
...
) -> Result<CapabilityIssuanceProof, KyriotesCsk2Error> {
...
pub fn issue_capability_and_commit<L: TransparencyLog>(
    ...
    epoch_cert: &EpochKeyCert,
    base_state: &AuthorityState,
) -> Result<(CapabilityIssuanceProof, TransparencyStateCommit), KyriotesCsk2Error> {
    if tree.authority_root() != base_state.authority_root {
        return Err(KyriotesCsk2Error::AuthorityState(
...
/// Open an Kyriotēs-CSK2 object while enforcing one or more [`CompromiseNotice`]s.
///
...
pub fn open_with_compromise_check_and_verifier<V: AuthorityVerifier>(
    ...
    let (e_req, wrapper) = select_required_wrapper(object, state)?;
    let verified_state = verifier.verify_state(state, &wrapper.transparency_proof)?;
    open_with_compromise_check_verified(
        recipient_sk,
        object,
        cap,
        proof,
...
#[allow(clippy::too_many_arguments)]
pub fn open_with_compromise_check_verified(
    recipient_sk: &RecipientSecretKey,
    object: &KyriotesCsk2Object,
    cap: &Capability,
    proof: &CapabilityProof,
...
pub fn verify_with_compromise_check_and_verifier<V: AuthorityVerifier>(
    ...
    let verified_state = verifier.verify_state(state, transparency_proof)?;
    verify_with_compromise_check_verified(object, cap, proof, &verified_state, notices)
}

pub fn verify_with_compromise_check_verified(
    object: &KyriotesCsk2Object,
    cap: &Capability,
    proof: &CapabilityProof,
    verified_state: &VerifiedAuthorityState,
...
pub fn rotate_epoch_and_commit<L: TransparencyLog>(
    ...
    validity_window: u64,
    prev_epoch_hash: &[u8; 32],
) -> Result<(EpochSigningKeyPair, EpochKeyCert, TransparencyStateCommit), KyriotesCsk2Error> {
...
pub struct EpochRotation {
    ...
    pub epoch_pk: [u8; 32],
    /// Epoch root signature: `Sign(sk_A_e, "KYRIOTES-CSK2-EPOCH-ROOT-v2" || …)` (§7).
    pub sigma_e: [u8; 64],
...
pub(crate) fn finalize_epoch_rotation_commit<L: TransparencyLog>(
    ...
    new_epoch: u64,
    prev_epoch_hash: &[u8; 32],
) -> Result<EpochRotation, KyriotesCsk2Error> {
...
mod crypto_contract_discharge_tests {
    use super::*;
    use crate::kyriotes_csk2::model::RecipientKeyPair;
    ...
    fn sample_object() -> KyriotesCsk2Object {
        KyriotesCsk2Object {
            version: 1,
            suite: "KYRIOTES-CSK2-DEV-CHACHA20POLY1305-HKDF-SHA256".to_string(),
