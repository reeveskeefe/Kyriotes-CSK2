...
///
/// - Hybrid (PQ present):  `H("KYRIOTES-CSK2-HYBRID-SECRET-v2" || ss_C || ss_Q)`
/// - Classical-only:       `H("KYRIOTES-CSK2-CLASSICAL-SECRET-v1" || ss_C)`
///
...
pub fn hybrid_secret(ss_c: &[u8; 32], ss_pq: Option<&[u8; 32]>) -> [u8; 32] {
    ...
    match ss_pq {
        Some(pq) => {
            h.update(b"KYRIOTES-CSK2-HYBRID-SECRET-v2");
            h.update(ss_c);
            h.update(pq);
        }
        None => {
            h.update(b"KYRIOTES-CSK2-CLASSICAL-SECRET-v1");
            h.update(ss_c);
        }
    }
    h.finalize().into()
