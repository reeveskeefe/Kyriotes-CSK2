...
/// A matched public/secret keypair for an Kyriotēs-CSK2 recipient.
...
pub struct Capability {
    /// Capability format version (spec §5). Must be 1 for Kyriotēs-CSK2 v0.1.
    pub version: u16,
    pub subject: String,
...
pub fn hash_policy(policy: &str) -> [u8; 32] {
    ...
    hasher.update(b"KYRIOTES-CSK2-POLICY-v1");
...
pub fn capability_leaf_hash(cap: &Capability) -> [u8; 32] {
    ...
    let mut hasher = Sha256::new();
    hasher.update(b"KYRIOTES-CSK2-CAPABILITY-LEAF-v1");
    hasher.update(enc);
    hasher.finalize().into()
}
...
pub fn capability_stamp(cap: &Capability, state: &AuthorityState) -> [u8; 32] {
    ...
    hasher.update(b"KYRIOTES-CSK2-CAPABILITY-STAMP-v2");
    hasher.update(leaf);
...
pub fn transparency_leaf_hash(state: &AuthorityState) -> [u8; 32] {
    ...
    let mut hasher = Sha256::new();
    hasher.update(b"KYRIOTES-CSK2-TRANSPARENCY-LEAF-v1");
    hasher.update(enc);
    hasher.finalize().into()
}
...
pub fn context_hash(
    ...
    let mut hasher = Sha256::new();
    hasher.update(b"KYRIOTES-CSK2-CONTEXT-v1");
    hasher.update(enc);
    hasher.finalize().into()
}
