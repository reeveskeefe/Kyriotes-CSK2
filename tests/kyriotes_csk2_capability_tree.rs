...
use kyriotes_csk2::{
    ...
    EpochSigningKeyPair, KyriotesCsk2Error, Rights, capability_leaf_hash,
    verify_capability_inclusion, verify_capability_issuance,
};
...
fn inclusion_proof_rejects_wrong_capability() {
    ...
    assert!(matches!(
        err,
        KyriotesCsk2Error::InvalidCapability("inclusion proof leaf hash does not match capability")
    ));
}
