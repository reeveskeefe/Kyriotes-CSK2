...
impl Setup {
    fn new(cap_epoch_start: u64, cap_epoch_end: u64, cert_epoch: u64, policy_label: &str) -> Self {
        ...
        let issuance_proof = issue_capability(&mut tree, &cap, &epoch_kp, &epoch_cert)
    ...
    fn build_proof(
        ...
        CapabilityProof {
            inclusion,
            non_revocation,
            issuance: self.issuance_proof.clone(),
        }
    }
...
/// Happy path: `seal_and_commit` returns an `KyriotesCsk2Object` and a
...
fn seal_and_commit_returns_valid_object_and_commit() {
    ...
    assert_ne!(
        ...
        "transparency root must be set after commit"
    );
    // Object must have exactly one wrapper.
    assert_eq!(
        object.wrappers.len(),
        1,
        "sealed object must have one wrapper"
    );
    // Wrapper epoch must match the state epoch.
    assert_eq!(object.wrappers[0].epoch, 42);
}

/// The wrapper baked into the `KyriotesCsk2Object` must contain the committed
...
fn seal_and_commit_wrapper_bakes_in_committed_proof() {
    ...
    assert_eq!(
        wrapper.transparency_proof.leaf_hash, commit.proof.leaf_hash,
...
fn seal_and_commit_object_is_immediately_openable() {
    ...
    let opened = open(&s.recipient.secret, &object, &cap, &proof, &commit.state)
...
fn seal_and_commit_idempotent_for_same_state() {
    ...
    assert_eq!(
        commit1.state.transparency_root, commit2.state.transparency_root,
...
fn seal_and_commit_rejects_insufficient_rights() {
    use kyriotes_csk2::{KyriotesCsk2Error, OpenRequest};
