...
use kyriotes_csk2::{
    AuthorityWrapper, DecodeLimits, DecodeProfile, InMemoryTransparencyLog, KyriotesCsk2Error,
    KyriotesCsk2Object, Rights, TemporalPolicy, TransparencyLog, TransparencyProof,
    decode_kyriotes_csk2_object, decode_kyriotes_csk2_object_with_limits, decode_profile_from_env,
    decode_profile_from_env_value, encode_kyriotes_csk2_object, seal,
};
...
#[test]
fn kyriotes_csk2_object_decode_rejects_bad_magic() {
    let mut bytes = encode_kyriotes_csk2_object(&kyriotes_csk2::KyriotesCsk2Object {
        version: 1,
        suite: "KYRIOTES-CSK2-DEV".to_string(),
        object_id: "o".to_string(),
        required_rights: kyriotes_csk2::Rights::READ,
        policy_hash: [1u8; 32],
        seal_epoch: 1,
    ...
    let err = decode_kyriotes_csk2_object(&bytes).expect_err("bad magic must fail");
    assert!(matches!(
        err,
        KyriotesCsk2Error::Parse("invalid Kyriotēs-CSK2 object magic")
    ));
}

#[test]
fn kyriotes_csk2_object_decode_rejects_truncated_input() {
    ...
    let err = decode_kyriotes_csk2_object(&encoded).expect_err("truncated payload should fail");
    assert!(matches!(err, KyriotesCsk2Error::Parse(_)));
}

#[test]
fn kyriotes_csk2_object_decode_rejects_oversized_payload_field() {
    let object = KyriotesCsk2Object {
        version: 1,
        suite: "KYRIOTES-CSK2-DEV".to_string(),
    ...
    let encoded = encode_kyriotes_csk2_object(&object);
    let err = decode_kyriotes_csk2_object(&encoded).expect_err("oversized payload must fail");
    assert!(matches!(
        err,
        KyriotesCsk2Error::Parse("field exceeds maximum allowed length")
    ));
}

#[test]
fn kyriotes_csk2_object_decode_rejects_oversized_wrapper_count() {
    ...
    let encoded = encode_kyriotes_csk2_object(&object);
    let err = decode_kyriotes_csk2_object(&encoded).expect_err("oversized wrapper count must fail");
    assert!(matches!(
        err,
        KyriotesCsk2Error::Parse("wrapper count exceeds maximum allowed")
    ));
}

#[test]
fn kyriotes_csk2_object_decode_rejects_oversized_wrapped_dek_field() {
    ...
    let encoded = encode_kyriotes_csk2_object(&object);
    let err = decode_kyriotes_csk2_object(&encoded).expect_err("oversized wrapped DEK must fail");
    assert!(matches!(
        err,
        KyriotesCsk2Error::Parse("field exceeds maximum allowed length")
    ));
...
#[test]
fn kyriotes_csk2_object_decode_with_limits_rejects_when_custom_max_payload_is_tighter() {
    let object = KyriotesCsk2Object {
        version: 1,
        suite: "KYRIOTES-CSK2-DEV".to_string(),
        object_id: "obj".to_string(),
    ...
    let err = decode_kyriotes_csk2_object_with_limits(&encoded, limits)
...
#[test]
fn kyriotes_csk2_object_decode_with_limits_allows_relaxed_wrapped_dek_limit() {
    let wrapper = AuthorityWrapper {
        epoch: 1,
        kem_ct_classical: [9u8; 32],
        kem_ct_pq: [8u8; 32].to_vec(),
    ...
    let decoded = decode_kyriotes_csk2_object_with_limits(&encoded, limits)
...
fn embedded_profile_rejects_large_payload() {
    ...
    let encoded = encode_kyriotes_csk2_object(&object);
    let err = decode_kyriotes_csk2_object_with_limits(&encoded, DecodeProfile::Embedded.limits())
...
fn server_profile_allows_larger_wrapped_dek() {
    ...
    let encoded = encode_kyriotes_csk2_object(&object);
    let decoded = decode_kyriotes_csk2_object_with_limits(&encoded, DecodeProfile::Server.limits())
        .expect("server profile should allow wrapped DEK up to 16 KiB");
    assert_eq!(decoded.wrappers[0].wrapped_dek.len(), 10 * 1024);
}

#[test]
fn kyriotes_csk2_object_decode_rejects_transparency_proof_with_too_many_siblings() {
    let mut log = InMemoryTransparencyLog::new();
    for i in 0..16u64 {
    ...
    assert!(matches!(
        err,
        KyriotesCsk2Error::Parse("transparency sibling count exceeds maximum allowed")
    ));
}
...
fn decode_profile_from_str_supports_env_cli_paths() {
    ...
    assert!(matches!(
        err,
        KyriotesCsk2Error::Parse("unknown decode profile")
    ));
}
...
fn decode_profile_from_env_missing_var_defaults_to_strict() {
    let profile = decode_profile_from_env("KYRIOTES_CSK2_DECODE_PROFILE_DOES_NOT_EXIST_FOR_TEST");
