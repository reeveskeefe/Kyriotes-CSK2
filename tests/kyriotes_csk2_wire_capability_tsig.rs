...
use kyriotes_csk2::{
    Capability, KyriotesCsk2Error, Rights, ThresholdPartialSig, ThresholdSignatureSet,
    decode_capability, decode_capability_with_limits, decode_threshold_signature_set,
...
fn capability_decode_rejects_trailing_bytes() {
    ...
    assert!(matches!(
        err,
        KyriotesCsk2Error::Parse("trailing bytes after capability")
    ));
}
...
fn tsig_set_decode_rejects_trailing_bytes() {
    ...
    assert!(matches!(
        err,
        KyriotesCsk2Error::Parse("trailing bytes after threshold signature set")
    ));
}
...
fn tsig_set_decode_rejects_count_exceeding_max() {
    ...
    assert!(matches!(
        err,
        KyriotesCsk2Error::Parse("partial signature count exceeds maximum allowed")
    ));
}
