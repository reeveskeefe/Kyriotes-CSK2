...
STATUS_PATH = PROJECT_ROOT / "tests" / "refinement" / "add_epoch_wrapper_full_proof_status.json"


def main() -> int:
    payload = json.loads(STATUS_PATH.read_text(encoding="utf-8"))

    assert payload["schema"] == "arc.add_epoch_wrapper.full_mechanical_proof_status.v1"
    assert payload["target"] == "src/kyriotes_csk2/engine.rs::add_epoch_wrapper_with_verifier"
    ...
    print("add_epoch_wrapper full proof status test passed.")
    return 0
