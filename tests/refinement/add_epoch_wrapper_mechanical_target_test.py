...
def main() -> int:
    ...
    assert payload["mechanically_checked_count"] >= 7
    assert payload["mechanically_proven_count"] == 0

    matches = [
        item for item in payload["entries"]
        if item["id"] == "engine.add_epoch_wrapper"
    ]
    ...
    assert "epoch-wrapper refinement track" in item["boundary"]

    print("add_epoch_wrapper mechanical target test passed.")
    return 0
