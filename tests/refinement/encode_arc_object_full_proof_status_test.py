...
STATUS_PATH = PROJECT_ROOT / "tests" / "refinement" / "encode_arc_object_full_proof_status.json"
...
def main() -> int:
    ...
    assert payload["schema"] in {
        "arc.encode_arc_object.full_mechanical_proof_status.v1",
        "arc.encode_arc_object.full_mechanical_proof_status.v2",
    }
    assert payload["target"] == "src/encoding/wire.rs::encode_arc_object"
    ...
    print("encode_arc_object full proof status test passed.")
    return 0
