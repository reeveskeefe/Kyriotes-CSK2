...
def main() -> int:
    ...
    assert payload["mechanically_checked_count"] >= 9
    assert payload["mechanically_proven_count"] == 0

    matches = [
        item for item in payload["entries"]
        if item["id"] == "engine.rotate_epoch_full"
    ]
    ...
    assert "full authority-rotation refinement track" in item["boundary"]

    print("rotate_epoch_full mechanical target test passed.")
    return 0
