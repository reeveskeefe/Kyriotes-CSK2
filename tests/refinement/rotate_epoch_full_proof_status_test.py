...
STATUS_PATH = PROJECT_ROOT / "tests" / "refinement" / "rotate_epoch_full_proof_status.json"


def main() -> int:
    payload = json.loads(STATUS_PATH.read_text(encoding="utf-8"))

    assert payload["schema"] == "arc.rotate_epoch.full_mechanical_proof_status.v1"
    assert payload["target"] == "src/kyriotes_csk2/engine.rs::rotated_authority_state"
    ...
    print("rotate_epoch full proof status test passed.")
    return 0
