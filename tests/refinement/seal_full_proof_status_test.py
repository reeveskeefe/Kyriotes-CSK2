...
STATUS_PATH = PROJECT_ROOT / "tests" / "refinement" / "seal_full_proof_status.json"


def main() -> int:
    payload = json.loads(STATUS_PATH.read_text(encoding="utf-8"))

    assert payload["schema"] == "arc.seal.full_mechanical_proof_status.v1"
    assert payload["target"] == "src/kyriotes_csk2/engine.rs::seal_with_verifier"
    ...
    print("seal full proof status test passed.")
    return 0
