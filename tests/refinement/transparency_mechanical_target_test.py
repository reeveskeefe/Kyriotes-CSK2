...
def main() -> int:
    ...
    assert payload["target_count"] == 11
    assert payload["mechanically_checked_count"] == 11
    assert payload["mechanically_proven_count"] == 0

    matches = [
        item for item in payload["entries"]
        if item["id"] == "transparency.append"
    ]
    ...
    assert "append/lookup/conflict refinement track" in item["boundary"]

    print("transparency mechanical target test passed.")
    return 0
