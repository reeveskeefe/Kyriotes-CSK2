...
STATUS_PATH = PROJECT_ROOT / "tests" / "refinement" / "verify_full_proof_status.json"


def main() -> int:
    payload = json.loads(STATUS_PATH.read_text(encoding="utf-8"))

    assert payload["schema"] == "arc.verify.full_mechanical_proof_status.v1"
    assert payload["target"] == "src/kyriotes_csk2/engine.rs::verify_with_verifier"
    ...
    print("verify full proof status test passed.")
    return 0
